    ...
    def suggestion(self, text: str) -> str:
        ...
        messages=[
            {"role": "developer", "content": "\
             用戶是一個日內交易者, 他主要的是做空股票的交易者, 用戶會提供最新的新聞的一些總結, 如果新聞中有非常強的正面情緒, 請向用戶列出風險, \
             解釋為何不建議做空, 但如果沒有當天的新聞, 或新聞中的正面情緒不高, 請向用戶列出建議。\
             如果用戶有提供原有的建議, 而原來的建議中沒有新聞的考慮, 請不要使用原有的建議, 而是重新生成一個新的建議。\
             如果用戶有提供原有的建議, 而原來的建議中有新聞的考慮, 請把原來的建議考慮進去, 並且在原來的建議中加上用戶新加入的新聞作為考慮, 生成一個新的縱合建議, \
             如果新聞有重覆, 請把重覆的人容合併差生成縱合建議。\
             如果新的新聞是亂碼, 或不明所意的內容, 和股票不相關, 請一字不改的輸出原來的建議, 你的回答會作為WEBAPP的直接輸出。"},
